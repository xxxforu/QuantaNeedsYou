<script setup>
	import { checkInList } from '../../api/checkIn.js'
	// var dataList = ref([{
	// 		'title': '2023年Quanta一面',
	// 		'interviewId': 0,
	...
	function goToDetail(e) {
		...
		uni.navigateTo({ url: '/pages/yiqianweiding-detail/yiqianweiding-detail?id=' + interviewid })
	}
	var dataList = ref()
	onLoad(() => {
...
<template>
	<view class="container">
		<view class="dataBox" v-for="(item,i) in dataList" :key="interviewId">
			...
			<!-- 开始时间 -->
			<view class="line">
				<image class="icon" src="../../static/image/startTime.png" mode="widthFix"></image>
				<text>{{item.startTime}}</text>
			</view>
			<!-- 未开始 -->
			<view class="line" v-if="item.state == 0" :data-interviewId="item.interviewId" @tap="goToDetail">
				<image class="icon" src="../../static/image/noStart.png" mode="widthFix"></image>
				<text>未开始</text>
				<image style="width: 20px;vertical-align: middle;float: right;" src="../../static/image/返回.png" mode="widthFix">
				</image>
			</view>
			<!-- 进行中 -->
			<view class="line" v-else-if="item.state == 1" :data-interviewId="item.interviewId" @tap="goToDetail">
				<image class="icon" src="../../static/image/endTime.png" mode="widthFix"></image>
			...
			<view class="line" v-else-if="item.state == 2" :data-interviewId="item.interviewId" @tap="goToDetail">
				<image class="icon" src="../../static/image/hasEnd.png" mode="widthFix"></image>
				<text>已结束</text>
				<image style="width: 20px;vertical-align: middle;float: right;" src="../../static/image/返回.png" mode="widthFix">
				</image>
			</view>
