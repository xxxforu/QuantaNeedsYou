<script setup>
	...
	onLoad(options => {
		...
		checkInDetail(options.id).then(res => {
			detail.value = res
			if (res.type === 2) {
				const val = uni.getStorageSync(detail.value.interviewId + 'state')
				if (val) {
					detail.value.type = val
				} else uni.setStorageSync(detail.value.interviewId + 'state', 2)
			}
	...
	function getDeque() {
		...
		uni.request({
			...
			checkList.value = []
			res.data.data.forEach(item => { //数据有效才保存
				if (item.departmentId !== null) {
					checkList.value.push(item)
				}
			})

			if (checkList.value.length === 0) { //判断处理完后数据列表是否为空
				errorNoList.value = true
			} else {
				errorNoList.value = false
				for (let i = 0; i < checkList.value.length; i++) { //如果存在'进行中'的数据 就弹出提示
				...
				scanCode({ code: result }).then(res => { //扫码签到
					switch (detail.value.type) {
						...
						case 1:
							...
							break
						case 2:
							uni.setStorageSync(detail.value.interviewId + 'state', 0)
							checkIn.value = 'false'
							successTip.value.open()
							detail.value.type = 0
							detail.value.buttonText = '面试签到'
					...
					if (err.data.msg.includes('过期')) checkIn.value = 'false' //当前码过期了
					else if (err.data.msg.includes('重复') && !uni.getStorageSync(detail.value.interviewId + 'state') ===
						2) {
						uni.setStorage({
							data: true,
							key: 'checkIn' + id
...
<template>
	<view class="container">
		...
		<view disabled="{{detail.state != 1}}" v-if="checkIn == 'false'"><button @tap="getScanCode"
				class="myButton">{{detail.buttonText}}</button></view>

		<view class="checkList" v-else>
			<view class="checkItem" v-for="(item,i) in checkList" :key="departmentId">
				<view class="content">
					<view class="left">
